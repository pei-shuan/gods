...
if (selectedTown) url.searchParams.append('town', selectedTown);
if (templeNameInput) url.searchParams.append('temple_name', templeNameInput);
...
function populateCitySelect() {
    fetch('get_cities.php') // 確保您的 API 可返回城市資料
        .then(response => response.json())
        .then(cities => {
            const citySelect = document.getElementById("citySelect");
            cities.forEach(city => {
                const option = document.createElement("option");
                option.value = city.id; // city_id
                option.textContent = city.name; // 顯示縣市名稱
                citySelect.appendChild(option);
            });
        })
        .catch(error => console.error('Error fetching cities:', error));
}
function populateTownSelect(cityId) {
    const townSelect = document.getElementById("townSelect");
    townSelect.disabled = !cityId; // 當未選取縣市時，鎖定下拉選單
    townSelect.innerHTML = '<option value="">請選擇鄉鎮市區</option>';

    if (cityId) {
        fetch(`get_towns.php?city_id=${cityId}`)
            .then(response => response.json())
            .then(towns => {
                if (towns.length > 0) {
                    towns.forEach(town => {
                        const option = document.createElement("option");
                        option.value = town.id; // 使用 API 回傳的 town id
                        option.textContent = town.name; // 顯示鄉鎮名稱
                        townSelect.appendChild(option);
                    });
                } else {
                    townSelect.innerHTML = '<option value="">無相關鄉鎮市區</option>';
                }
            })
            .catch(error => {
...
function fetchTemples(cityId = 0, townName = '') {
    const url = new URL('get_temples.php', window.location.href);
    if (cityId) url.searchParams.append('city_id', cityId);
    if (townName) url.searchParams.append('town_name', townName);
        ...
        .then(locations => {
            ...
            markers = locations.map(location => {
                ...
                const infowindow = new google.maps.InfoWindow({
                    content: `<div><h1>${location.name}</h1><p>地址: ${location.address}</p><br><a href="temple_info.html">詳細資料</a></div>`,
                });
...
document.getElementById("search-form").addEventListener("submit", (event) => {
    ...
    const templeName = document.getElementById("search-input").value.trim();
    if (templeName) {
        fetchTempleByName(templeName); // 呼叫搜尋函數
    } else {
        alert("請輸入廟宇名稱！");
    }
});
function fetchTempleByName(name) {
    const url = new URL('get_temples.php', window.location.href);
    ...
    fetch(url)
        ...
        .catch(error => console.error('Error fetching temple by name:', error));
}
